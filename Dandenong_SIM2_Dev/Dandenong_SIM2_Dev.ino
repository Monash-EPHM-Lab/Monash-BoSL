...
void SendToWeb222(int abc) {
  ...
  a=0;
  do{a++;}while (sendATcommand(F("AT+CGDCONT=1,\"IP\",\"yesinternet\",\"0.0.0.0\""), "OK",10000) == 0 && a<5);
  a=0;
  do{a++;}while (sendATcommand(F("AT+CGSOCKCONT=1,\"IP\",\"yesinternet\""), "OK",10000) == 0 && a<5);
  a=0;
...
//you may need to move the /EoDC/ to the below line bewfore the "WriteMe.php... so it looks like "EoDC/WriteMe.php.... but try the way i have it first
  ...
  if(abc == 0){
  AAA = "GET /";
  AAA += "EoDC/databases/WriteMe.php?SiteName=development.csv&T=";
  AAA += "normal&EC=";
  int transmitVoltage = batlv.readVolt();
  String voltageString = String(transmitVoltage);
  AAA += voltageString;
