...
uint8_t ECPinA = A1;  //EC +- pin
uint8_t ECPinB = A3;  //EC +- pin
...
uint8_t reps = 0;
float lastTemp = 1, lastEC = 1;
double lastPressure = 1;  // last true pressure reading. sensor pressure - air pressure
float tempSum = 0, ECSum = 0;
double sensorPressureSum = 0, airPressureSum = 0;
uint16_t batLevel = 4200; //mV
bool polarity = false; // polarity of EC pins
...
void initialiseStrings() {  // we initialise strings in flash using some special functions.
	...
	sitename = F("BoSLv0_2_test2.csv");  // sitename 
}
...
void setup() {
  ...
  pinMode(ECPinR, INPUT); //EC read
  pinMode(ECPinA, OUTPUT);//EC Power
  pinMode(ECPinB,OUTPUT);//EC Ground 
  digitalWrite(ECPinA,LOW);
  digitalWrite(ECPinB,LOW);//EC Ground is set to low       
  ...
  bosl.setFunctionality(1); // Set modem to full functionality
...
void loop() {
	...
	//ECSum += measureEC(); //take measurement every 10sec
...
uint16_t readEC(){
  digitalWrite(ECPinA,HIGH);
  digitalWrite(ECPinB,LOW);

  delay(1); //this value may need to be adjusted
  ...
  uint16_t ECVal = analogRead(ECPinR);
  ECVal = analogRead(ECPinR);
